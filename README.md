...
- [ ] 과정, 레벨, 미션 안내 문구 출력
- [ ] 페어 매칭 결과 출력 (`:` 구분자 사용)
- [ ] 재매칭 안내 문구 출력 ("매칭 정보가 있습니다. 다시 매칭하시겠습니까?")
- [ ] 초기화 완료 메시지 출력
...
#### 입력 검증
- [ ] 기능 선택 입력값 검증 (1, 2, 3, Q 외 입력 시 예외)
- [ ] 과정 입력값 검증 (백엔드, 프론트엔드 외 입력 시 예외)
- [ ] 레벨 입력값 검증 (레벨1~5 외 입력 시 예외)
- [ ] 미션 입력값 검증 (해당 레벨에 없는 미션 입력 시 예외)
- [ ] 재매칭 입력값 검증 (네, 아니오 외 입력 시 예외)
- [ ] 입력 형식 검증 (쉼표와 공백으로 올바르게 구분되었는지)
...
- [X] 문자열로 Course enum 찾는 정적 팩토리 메서드
- [X] 입력값이 다를 경우 예외 발생
...
- [X] 레벨별 매칭된 페어 조합 저장
- [X] 같은 레벨에서 이미 매칭된 조합인지 확인
- [ ] 특정 레벨의 매칭 이력 반환
- [ ] 전체 매칭 이력 초기화

### 파일 처리
- [ ] backend-crew.md 파일 읽기
- [ ] frontend-crew.md 파일 읽기
- [ ] 파일에서 크루 이름 목록 파싱 (List<String>)
- [ ] 파일 읽기 실패 시 예외 처리

### 페어 매칭
#### 매칭 전 검증
- [ ] 이미 해당 미션에 매칭이 있는지 확인
- [ ] 매칭이 있을 경우 재매칭 여부 확인

#### 매칭 로직
- [ ] 과정 기반으로 크루 목록 불러오기
- [ ] Randoms.shuffle()을 통해 크루 목록 섞기
- [ ] 섞인 목록에서 앞에서부터 2명씩 페어 생성
- [ ] 홀수인 경우 마지막 크루를 마지막 페어에 추가 (3명 페어)
- [ ] 생성된 페어들의 유효성 검증

#### 재매칭 로직
- [ ] 같은 레벨에서 이미 매칭된 조합인지 검증
- [ ] 중복 조합 발견 시 다시 섞기
- [ ] 최대 3회까지 재시도
- [ ] 3회 실패 시 에러 메시지 출력
...
### 페어 조회
- [ ] Course, Level, Mission으로 매칭 결과 조회
...
### 예외 처리
- [ ] 잘못된 기능 선택 시 예외
- [ ] 잘못된 과정/레벨/미션 입력 시 예외
- [ ] 매칭 불가능한 경우 예외 (3회 시도 후)
- [ ] 파일 읽기 실패 시 예외
- [ ] 모든 예외는 IllegalArgumentException으로 통일
- [ ] 예외 메시지는 "[ERROR]"로 시작
- [ ] 예외 발생 시 해당 입력부터 재시도
